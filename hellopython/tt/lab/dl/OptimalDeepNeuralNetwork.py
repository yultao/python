...
Test cases
"""
class OptimalDeepNeuralNetworkTestCase():
...
class OptimalDeepNeuralNetwork():
...
#         print(parameters["W1"][:,0:10])
        cost = -(1/m) * np.sum(     Y * log1 +   (1-Y) * log2  ) 
...
#         L2_regularization_cost = (1/m)*(lambd/2)*(np.sum(np.square(W1)) + np.sum(np.square(W2)) + np.sum(np.square(W3)))
    ...
    def linear_backward_with_regularization(self, dZ, cache, lambd):
        """
        Implement the linear portion of backward propagation for a single layer (layer l)
    ...
    def linear_activation_backward_with_regularization(self, dA, cache, activation, lambd):
        """
        Implement the backward propagation for the LINEAR->ACTIVATION layer.
        ...
        if activation == "relu":
            ### START CODE HERE ### (≈ 2 lines of code)
            dZ = self.relu_backward(dA, activation_cache)
            dA_prev, dW, db = self.linear_backward_with_regularization(dZ, linear_cache, lambd)
            ### END CODE HERE ###
    ...
    def L_model_backward_with_regularization(self, AL, Y, caches, lambd):
        """
        Implement the backward propagation for the [LINEAR->RELU] * (L-1) -> LINEAR -> SIGMOID group
        ...
        ### START CODE HERE ### (approx. 2 lines)
        current_cache = caches[L-1]
        grads["dA" + str(L)], grads["dW" + str(L)], grads["db" + str(L)] = self.linear_activation_backward_with_regularization(dAL, current_cache, "sigmoid", lambd)
        ### END CODE HERE ###
        ...
        for l in reversed(range(L-1)):
            ...
            ### START CODE HERE ### (approx. 5 lines)
            current_cache = caches[l]
            dA_prev_temp, dW_temp, db_temp = self.linear_activation_backward_with_regularization(grads["dA" + str(l + 2)], current_cache, "relu", lambd)
            grads["dA" + str(l + 1)] = dA_prev_temp
            grads["dW" + str(l + 1)] = dW_temp
            grads["db" + str(l + 1)] = db_temp
            ### END CODE HERE ###
    ...
    def predict_mnist(self, X, y, parameters):
        """
        This function is used to predict the results of a  L-layer neural network.
    ...
    # GRADED FUNCTION: L_layer_model
    def L_layer_model_with_regularization(self, X, Y, layers_dims, learning_rate = 0.0075, num_iterations = 3000, print_cost=False, lambd=0):#lr was 0.009
        """
        Implements a L-layer neural network: [LINEAR->RELU]*(L-1)->LINEAR->SIGMOID.
        ...
        for i in range(0, num_iterations):
            ...
            # Backward propagation.
            ### START CODE HERE ### (≈ 1 line of code)
            grads = self.L_model_backward_with_regularization(AL, Y, caches, lambd)
            ### END CODE HERE ###
    ...
    # GRADED FUNCTION: L_layer_model
    def L_layer_model_with_regularization_minibatch(self, X, Y, layers_dims, learning_rate = 0.0075, 
...
#                 print(str(num_mb)+"/"+str(num_minibatches))
                ...
                # Compute cost.
                ### START CODE HERE ### (≈ 1 line of code)
                cost = self.compute_cost_with_regularization(AL, minibatch_Y, parameters, lambd)
                ### END CODE HERE ###
            
                # Backward propagation.
                ### START CODE HERE ### (≈ 1 line of code)
                grads = self.L_model_backward_with_regularization(AL, minibatch_Y, caches, lambd)
                ### END CODE HERE ###
...
def testOptimalMinibatchMnist():
    dnn = OptimalDeepNeuralNetwork()
    dnntc= OptimalDeepNeuralNetworkTestCase()
...
#     print(test_y.shape)
    ...
    print("\n===================L_layer_model_with_regularization_minibatch \n")
    parameters = dnn.L_layer_model_with_regularization_minibatch(
        train_x, train_y, layers_dims,  print_cost = True, 
        lambd = 0, learning_rate = 0.5, 
        mini_batch_size=64, num_epochs=30,
        print_cost_interval=1)
     
    print("\n===================predict train\n")
    pred_train = dnn.predict_mnist(train_x, train_y, parameters)
     
#     print("\n===================predict test\n")
#     pred_test = dnn.predict(test_x, test_y, parameters)

def testOptimalMinibatchCat():
    dnn = OptimalDeepNeuralNetwork()
    dnntc= OptimalDeepNeuralNetworkTestCase()
    
    print("\n===================load_data\n")
    train_x_orig, train_y, test_x_orig, test_y, classes = dnntc.load_cat_data()
...
# test case 1
    print("\n===================L_layer_model lambd=0, num_iterations = 2500\n")
    parameters = dnn.L_layer_model_with_regularization(train_x, train_y, layers_dims, num_iterations = 2500, print_cost = True, lambd = 0)
...
# test case 2
    print("\n===================L_layer_model lambd=0, mini_batch_size=m, num_epchos=2500\n")
    parameters = dnn.L_layer_model_with_regularization_minibatch(train_x, train_y, layers_dims,  print_cost = True, lambd = 0, mini_batch_size=209, num_epochs=2500)
...
def testOriginalAll():
    ...
    print("\n===================load_data\n")
    train_x_orig, train_y, test_x_orig, test_y, classes = dnn.load_cat_data()
