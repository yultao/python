...
class NNMnistTestCase():
    def load_q_data(self):
        train_data = np.loadtxt("./datasets/train_q.txt", delimiter=',', skiprows=0)
        m,n = train_data.shape
        train_set_x_orig = train_data[:, 0].reshape(1,m)
        train_set_y_orig = train_data[:, 1].reshape(1,m)
#         plt.scatter(train_set_x_orig, train_set_y_orig, marker="x", color="red")
#         plt.show();

        test_data = np.loadtxt("./datasets/test_q.txt", delimiter=',', skiprows=0)
        m,n = test_data.shape
        test_set_x_orig = test_data[:, 0].reshape(1,m)
        test_set_y_orig = test_data[:, 1].reshape(1,m)
...
class NNMnist():
    ...
    def L_model_forward(self, X, parameters):
        ...
        ### START CODE HERE ### (≈ 2 lines of code)
        AL, cache = self.linear_activation_forward(A, parameters['W' + str(L)], parameters['b' + str(L)], "relu")
        caches.append(cache)
        ### END CODE HERE ###
...
def testOptimalMinibatchMnist():
    ...
    train_x, train_y,test_x, test_y = dnntc.load_q_data()
    layers_dims =  [1, 2,  1]
    parameters = dnn.L_layer_model(
        train_x, train_y, layers_dims,  print_cost = True, 
        lambd = 0, learning_rate = 1, 
        mini_batch_size=10, num_epochs=50,
        print_cost_interval=1)
    
    dnn.predict(test_x, test_y, parameters)
